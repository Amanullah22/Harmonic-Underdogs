...
if(!empty($_SESSION['email'])){
	...
	if($email_friend->num_rows==0)
		...
		FROM users
		WHERE talent='Drummer' AND email <> '$email'");
		$record = array();
		...
		$prj= mysqli_query($con,"SELECT *
		FROM users
		WHERE email NOT IN ($result) AND talent='Drummer' AND email <> '$email'");
		$record = array();
        while($row = mysqli_fetch_assoc($prj)){
            $record[] = $row;
        }}
...
<div class="w3-row" id="about">
  ...
  <div class="w3-half w3-white w3-container w3-center" style="height:620px;margin-top:40px;">
    ...
    <div>
      <a href="Pianists.php" class="w3-btn w3-btn-block w3-hover-teal w3-padding-16">Pianists&nbsp;&nbsp;<i class="fa fa-angle-double-right"></i></a>
    </div>
	<div>
      <a href="Guitarists.php" class="w3-btn w3-btn-block w3-hover-teal w3-padding-16"><i class="fa fa-angle-double-left"></i>&nbsp;&nbsp;Guitarists</a>
    </div>
  </div>
</div>
